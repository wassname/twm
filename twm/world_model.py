...
class ObservationModel(nn.Module):
    ...
    def create_z_dist(logits: Logits, temperature=1) -> Z_dist:
        ...
        return D.Independent(
            D.OneHotCategoricalStraightThrough(logits=logits / temperature), 1
        )
    ...
    def encode(self, o: Obs) -> Z_dist:
        ...
        z_logits = z_logits.unflatten(
            -1, (config["z_categoricals"], config["z_categories"])
        )
    ...
    def compute_decoder_loss(self, recons: Obs, o: Obs):
        ...
        if coef != 0:
            ...
            recon_dist = D.Independent(
                D.Normal(recon_mean, torch.ones_like(recon_mean)), 3
            )
    ...
    def compute_entropy_loss(self, z_dist: Z_dist):
        ...
        if coef != 0:
            if config["obs_entropy_threshold"] < 1:
                ...
                loss = coef * torch.relu(
                    config["obs_entropy_threshold"] - normalized_entropy
                )
            else:
            ...
            loss = torch.zeros(
                1, device=z_dist.base_dist.logits.device, requires_grad=False
            )
...
class DynamicsModel(nn.Module):
    def __init__(self, config, z_dim, num_actions):
        ...
        out_heads = {
            ...
            "r": {
                "hidden_dims": config["dyn_reward_dims"],
                "out_dim": 1,
                "final_bias_init": 0.0,
            },
            "g": {
    ...
    def compute_dynamics_loss(
        self, preds, h, target_logits, target_r, target_g, target_weights
    ):
